...
export function useLocalStorage<T>(key: string, initialValue: T | (() => T)) {
  ...
  useEffect(() => {
    // This effect runs once on mount on the client side to correctly initialize state from localStorage
    setStoredValue(getInitialValue(key, initialValue));
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [key]); // Only re-run if key changes, initialValue is captured by getInitialValue
