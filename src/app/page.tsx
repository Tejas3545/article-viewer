...
export default function HomePage() {
  ...
  const handleLogout = async () => {
    try {
      await signOut(auth);
      // Optionally, show a toast or redirect handled by useAuth
    } catch (error) {
    ...
    <div className="container mx-auto p-4 md:p-8 space-y-10">
      ...
      <div className="flex justify-end">
        <Button
          ...
          className="logout-animate bg-gradient-to-r from-pink-500 via-red-500 to-yellow-500 text-white font-bold shadow-lg hover:scale-110 hover:shadow-2xl transition-transform duration-300 focus:scale-95 focus:outline-none focus:ring-4 focus:ring-pink-300"
        >
          Log Out
        </Button>
      </div>
